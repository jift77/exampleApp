...
namespace ExampleApp.Infraestructure
{
    public class NumbersBinder : IModelBinder
    {
        ...
        private Numbers CreateInstance(Dictionary<string, ValueProviderResult> data)
        {
            ...
            {
                Op = new Operation { Add = Convert<bool>(data["add"]), Double = Convert<bool>(data["double"]) },
                Accept = Convert<string>(data["accept"])
            };
        }

        private ValueProviderResult GetValue(ModelBindingContext bindingContext, params string[] names)
        {
            for (int i = 0; i < names.Length - 1; i++)
            {
                string prefix = string.Join(".", names.Skip(i).Take(names.Length - (i + 1)));
                if (prefix != string.Empty && bindingContext.ValueProvider.ContainsPrefix(prefix))
                    return bindingContext.ValueProvider.GetValue(prefix + "." + names.Last());
            }
            return bindingContext.ValueProvider.GetValue(names.Last());
        }
